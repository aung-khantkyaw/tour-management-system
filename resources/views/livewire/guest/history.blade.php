<x-layouts.guest title="History - Tour Management System">
    <div class="max-w-5xl mx-auto px-4 py-16">
        ...
        <p class="text-center text-gray-600 max-w-2xl mx-auto mb-12">View all your tour bookings and their current status.</p>
        ...
        @if($bookings->isEmpty())
            <div class="text-center py-12">
                ...
                <a href="{{ route('packages') }}" class="inline-flex items-center px-6 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition">
                    Browse Packages
                </a>
            </div>
        @else
                ...
                @foreach($bookings as $booking)
                    <div class="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
                        <div class="flex flex-wrap items-start justify-between gap-4 mb-4">
                            <div class="flex-1">
                                <h3 class="font-semibold text-gray-900 text-lg">{{ $booking->schedule->touristPackage->package_name ?? 'Package' }}</h3>
                            ...
                            <div class="flex flex-col items-end gap-2">
                                <div class="flex gap-2">
                                    <span class="text-xs inline-flex items-center px-2 py-1 rounded-full font-medium
                                    ...
                                    <span class="text-xs inline-flex items-center px-2 py-1 rounded-full font-medium
                                        {{ $booking->booking_status === 'confirmed' ? 'bg-blue-100 text-blue-800' : 
                                           ...
                                           ($booking->booking_status === 'completed' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800')) }}">
                        ...
                        <div class="border-t pt-4">
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                                <div>
                                    ...
                                    <span class="font-medium text-gray-900">BK-{{ str_pad($booking->booking_id, 6, '0', STR_PAD_LEFT) }}</span>
                                </div>
                                <div>
                                ...
                                <div>
                                    ...
                                    <span class="font-medium text-gray-900">{{ $booking->roomChoices->first()?->accommodation?->hotel?->name ?? 'N/A' }}</span>
                                </div>
                            </div>
