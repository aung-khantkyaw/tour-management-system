                ...
                <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
                    <div class="space-y-2">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <h1 class="text-3xl font-bold bg-gradient-to-r from-gray-900 to-gray-700 bg-clip-text text-transparent">Schedules Management</h1>
                ...
                <div class="bg-white rounded-2xl shadow-lg border border-gray-100 p-6 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center">
                            ...
                            <div class="ml-5">
                                <p class="text-sm font-semibold text-gray-500 uppercase tracking-wide">Total Schedules</p>
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center">
                            ...
                            <div class="ml-5">
                                ...
                                <p class="text-3xl font-bold text-gray-900 mt-1">{{ $schedules->sum('bookings_count') }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center">
                            ...
                            <div class="ml-5">
                                ...
                                <p class="text-3xl font-bold text-gray-900 mt-1">{{ $schedules->where('from_date', '<=', now())->where('to_date', '>=', now())->count() }}</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                @forelse($schedules as $schedule)
                    <div class="bg-white rounded-2xl border border-gray-100 shadow-lg overflow-hidden hover:shadow-2xl hover:border-green-200 transition-all duration-300 transform hover:-translate-y-2 group"
                        ...
                        x-cloak data-package="{{ $schedule->touristPackage->package_name ?? '' }}" data-destination="{{ $schedule->touristPackage->destination->destination_name ?? '' }}">
                        
                        <!-- Schedule Header -->
                        <div class="relative h-48 bg-gradient-to-br from-green-100 via-emerald-100 to-teal-100 overflow-hidden">
                            ...
                            <div class="absolute top-4 right-4">
                                ...
                                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold 
                                    ...
                                    <span class="w-2 h-2 {{ $isActive ? 'bg-green-400' : ($isPast ? 'bg-gray-400' : 'bg-blue-400') }} rounded-full mr-2 animate-pulse"></span>
                            ...
                            <div class="absolute top-4 left-4">
                                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-white/90 text-gray-800 shadow-sm">
                                    ...
                                    {{ \Carbon\Carbon::parse($schedule->from_date)->diffInDays($schedule->to_date) + 1 }} Days
                                </span>
                            </div>
                        </div>
                            ...
                            <div class="mb-4">
                                ...
                                <div class="space-y-2">
                                    <div class="flex items-center text-gray-600">
                                        ...
                                        <span class="text-sm font-medium">{{ $schedule->touristPackage->destination->destination_name ?? 'No destination' }}</span>
                                    </div>
                                    <div class="flex items-center text-gray-600">
                                        <svg class="w-4 h-4 mr-2 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/>
                                        </svg>
                                        <span class="text-sm font-medium">{{ $schedule->touristPackage->guide->gname ?? 'No guide assigned' }}</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="mb-4 p-4 bg-gradient-to-r from-gray-50 to-gray-100 rounded-xl">
                                <div class="flex justify-between items-center">
                                    <div>
                                        ...
                                        <p class="text-lg font-bold text-gray-900">{{ \Carbon\Carbon::parse($schedule->from_date)->format('M d, Y') }}</p>
                                    </div>
                                    <div class="text-right">
                                        <p class="text-sm text-gray-600">To</p>
                                        <p class="text-lg font-bold text-gray-900">{{ \Carbon\Carbon::parse($schedule->to_date)->format('M d, Y') }}</p>
                                    </div>
                                </div>
                            </div>
                ...
                @empty
                    <div class="col-span-full">
                        <div class="bg-white rounded-2xl shadow-lg border border-gray-100 p-12 text-center">
                            ...
                            <p class="text-gray-600 mb-8 max-w-md mx-auto">Get started by creating your first schedule. Set up tour dates and times for your packages.</p>
                            <a href="{{ route('admin.schedules.create') }}"
