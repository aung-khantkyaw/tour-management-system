                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center">
                            ...
                            <div class="ml-5">
                                ...
                                <p class="text-3xl font-bold text-gray-900 mt-1">
                                    ${{ number_format($bookings->sum('total_amount'), 0) }}</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                @forelse($bookings as $booking)
                            <div class="bg-white rounded-2xl border border-gray-100 shadow-lg overflow-hidden hover:shadow-2xl hover:border-orange-200 transition-all duration-300 transform hover:-translate-y-2 group"
                                ...
                                x-cloak data-customer="{{ $booking->user->name ?? '' }}"
                                data-package="{{ $booking->schedule->touristPackage->package_name ?? '' }}">

                                <!-- Booking Header -->
                                <div
                                    class="relative h-48 bg-gradient-to-br from-orange-100 via-red-100 to-pink-100 overflow-hidden">
                    ...
                    ($booking->booking_status === 'pending' ? 'bg-yellow-100 text-yellow-800' :
                        ($booking->booking_status === 'cancelled' ? 'bg-red-100 text-red-800' : 'bg-gray-100 text-gray-800')) }} shadow-sm">
                                            <span
                                                class="w-2 h-2 {{ $booking->booking_status === 'confirmed' ? 'bg-green-400' :
                    ($booking->booking_status === 'pending' ? 'bg-yellow-400' :
                        ($booking->booking_status === 'cancelled' ? 'bg-red-400' : 'bg-gray-400')) }} rounded-full mr-2 animate-pulse"></span>
                                ...
                                <div class="p-6">
                                    ...
                                    <div class="mb-4">
                                        <h3
                                            ...
                                            {{ $booking->user->name ?? 'Unknown Customer' }} <span
                                        ...
                                        <div class="space-y-2">
                                            <div class="flex items-center text-gray-600">
                                                ...
                                                <span
                                                    class="text-sm font-medium">{{ $booking->schedule->touristPackage->package_name ?? 'No package' }}</span>
                                            </div>
                                            <div class="flex items-center text-gray-600">
                                                <svg class="w-4 h-4 mr-2 text-gray-400" fill="none" stroke="currentColor"
                                                    viewBox="0 0 24 24">
                                                ...
                                                <span
                                                    class="text-sm font-medium">{{ $booking->schedule->touristPackage->destination->destination_name ?? 'No destination' }}</span>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="mb-4 p-4 bg-gradient-to-r from-gray-50 to-gray-100 rounded-xl">
                                        <div class="flex justify-between items-center mb-2">
                                            ...
                                            <div class="text-right">
                                                ...
                                                <p class="text-lg font-bold text-gray-900">
                                                    ${{ number_format($booking->total_amount) }}</p>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="grid grid-cols-2 gap-4 mb-6">
                                        <div class="text-center p-3 bg-orange-50 rounded-xl border border-orange-100">
                                            <p class="text-sm font-bold text-orange-600">{{ ucfirst($booking->payment_method) }}</p>
                                        ...
                                        <div class="text-center p-3 bg-blue-50 rounded-xl border border-blue-100">
                                            <p class="text-sm font-bold text-blue-600">{{ ucfirst($booking->package_type) }}</p>
                ...
                @empty
                    <div class="col-span-full">
                        <div class="bg-white rounded-2xl shadow-lg border border-gray-100 p-12 text-center">
                            ...
                            <p class="text-gray-600 mb-8 max-w-md mx-auto">Bookings will appear here when customers make
                                reservations for tour packages.</p>
                        </div>
                    </div>
                @endforelse
            </div>
