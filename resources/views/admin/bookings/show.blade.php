                ...
                <div class="lg:col-span-2 space-y-8">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    <label class="text-sm font-semibold text-gray-500 uppercase tracking-wide">Full
                                        Name</label>
                                    <p class="text-lg font-bold text-gray-900 mt-1">{{ $booking->user->name ?? 'N/A' }}
                                    </p>
                                </div>
                                <div>
                                    <label
                                        class="text-sm font-semibold text-gray-500 uppercase tracking-wide">Email</label>
                                    <p class="text-lg font-bold text-gray-900 mt-1">{{ $booking->user->email ?? 'N/A' }}
                                    </p>
                                </div>
                                <div>
                                    <label
                                        class="text-sm font-semibold text-gray-500 uppercase tracking-wide">Phone</label>
                                    <p class="text-lg font-bold text-gray-900 mt-1">{{ $booking->phone }}</p>
                                </div>
                                <div>
                                    <label
                                        class="text-sm font-semibold text-gray-500 uppercase tracking-wide">Nationality</label>
                    ...
                    <div class="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
                        ...
                        <div class="p-8">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    <label class="text-sm font-semibold text-gray-500 uppercase tracking-wide">Package
                                        Name</label>
                                    <p class="text-lg font-bold text-gray-900 mt-1">
                                        {{ $booking->schedule->touristPackage->package_name ?? 'N/A' }}</p>
                                </div>
                                <div>
                                    <label
                                        class="text-sm font-semibold text-gray-500 uppercase tracking-wide">Destination</label>
                                    <p class="text-lg font-bold text-gray-900 mt-1">
                                        {{ $booking->schedule->touristPackage->destination->destination_name ?? 'N/A' }}
                                    </p>
                                </div>
                                <div>
                                    <label
                                        class="text-sm font-semibold text-gray-500 uppercase tracking-wide">Guide</label>
                                    <p class="text-lg font-bold text-gray-900 mt-1">
                                        {{ $booking->schedule->touristPackage->guide->gname ?? 'N/A' }}</p>
                                </div>
                                <div>
                                    <label
                                        class="text-sm font-semibold text-gray-500 uppercase tracking-wide">Duration</label>
                                    <p class="text-lg font-bold text-gray-900 mt-1">
                                        {{ $booking->schedule->touristPackage->duration_days ?? 'N/A' }} days</p>
                                </div>
                                <div>
                                    <label class="text-sm font-semibold text-gray-500 uppercase tracking-wide">From
                                        Date</label>
                                    <p class="text-lg font-bold text-gray-900 mt-1">
                                        {{ \Carbon\Carbon::parse($booking->schedule->from_date)->format('M d, Y') }}</p>
                                </div>
                                <div>
                                    <label class="text-sm font-semibold text-gray-500 uppercase tracking-wide">To
                                        Date</label>
                                    <p class="text-lg font-bold text-gray-900 mt-1">
                                        {{ \Carbon\Carbon::parse($booking->schedule->to_date)->format('M d, Y') }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                            ...
                            @forelse($booking->roomChoices as $roomChoice)
                                <div class="mb-4 p-4 bg-gray-50 rounded-lg">
                                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                        <div>
                                            <label
                                                class="text-sm font-semibold text-gray-500 uppercase tracking-wide">Hotel</label>
                                            <p class="text-lg font-bold text-gray-900 mt-1">
                                                {{ $roomChoice->accommodation->hotel->name ?? 'N/A' }}</p>
                                        </div>
                                        <div>
                                            <label class="text-sm font-semibold text-gray-500 uppercase tracking-wide">Room
                                                Type</label>
                                            <p class="text-lg font-bold text-gray-900 mt-1">
                                                {{ $roomChoice->accommodation->room->room_type ?? 'N/A' }}</p>
                                        </div>
                                        <div>
                                            <label
                                                class="text-sm font-semibold text-gray-500 uppercase tracking-wide">Price</label>
                                            <p class="text-lg font-bold text-gray-900 mt-1">
                                                ${{ number_format($roomChoice->accommodation->price ?? 0, 2) }}</p>
                                        </div>
                                    </div>
                                </div>
                            @empty
                            ...
                            <form action="{{ route('admin.bookings.updateStatus', $booking) }}" method="POST"
                                ...
                                <div>
                                    ...
                                    <select name="booking_status"
                                        class="w-full border-2 border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition-all duration-200">
                                        ...
                                        <option value="refunded" {{ $booking->booking_status === 'refunded' ? 'selected' : '' }}>Refunded</option>
                                    </select>
                                </div>

                                <button type="submit"
                                    class="w-full inline-flex items-center justify-center gap-2 px-4 py-3 bg-gradient-to-r from-orange-600 to-red-600 text-white rounded-xl hover:from-orange-700 hover:to-red-700 transition-all duration-200 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 font-semibold">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M5 13l4 4L19 7" />
                                    </svg>
                    ...
                    <div class="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
                        ...
                        <div class="p-6 space-y-4">
                            ...
                            <div class="flex justify-between items-center">
                                ...
                                <span
                                    class="font-bold text-gray-900">{{ \Carbon\Carbon::parse($booking->booking_date)->format('M d, Y') }}</span>
                            </div>
                            <div class="flex justify-between items-center">
    ...
    ($booking->booking_status === 'pending' ? 'bg-yellow-100 text-yellow-800' :
        ($booking->booking_status === 'in_progress' ? 'bg-blue-100 text-blue-800' :
            ($booking->booking_status === 'completed' ? 'bg-purple-100 text-purple-800' :
                ($booking->booking_status === 'refunded' ? 'bg-orange-100 text-orange-800' : 'bg-red-100 text-red-800')))) }}">
                            ...
                            @if($booking->payment_transaction_id)
                                <div class="flex justify-between items-center">
                                    ...
                                    <span
                                        class="font-bold text-gray-900 text-sm">{{ $booking->payment_transaction_id }}</span>
                                </div>
                            @endif
                            <div class="border-t pt-4">
                                <div class="flex justify-between items-center">
                                    ...
                                    <span
                                        class="text-2xl font-bold text-orange-600">${{ number_format($booking->total_amount) }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
